...
//    }
    ...
    label1.text=[NSString stringWithFormat:@"游玩时长:%@\n开放时机:%@\n交通信息:%@",[infoDic objectForKey:@"h"],[infoDic objectForKey:@"i"],[infoDic objectForKey:@"j"]];
    label1.font=[UIFont systemFontOfSize:12];
    label1.numberOfLines=0;
    [self.view addSubview:label1];
    
    menpiao=[UIButton buttonWithType:UIButtonTypeCustom];
    menpiao.frame=CGRectMake(0, label1.frame.origin.y+label1.frame.size.height, self.view.bounds.size.width, 40);
    [menpiao setBackgroundImage:[UIImage imageNamed:@"nearby_detail_line"] forState:UIControlStateNormal];
    [menpiao setTitle:[NSString stringWithFormat:@"   门票：%@",[infoDic objectForKey:@"k"]] forState:UIControlStateNormal];
    [menpiao setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    menpiao.contentHorizontalAlignment=UIControlContentHorizontalAlignmentLeft;
    menpiao.titleLabel.font=[UIFont systemFontOfSize:13];
    [menpiao addTarget:self action:@selector(menpiaoButton) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:menpiao];
    
    UIImageView *accessory1=[[UIImageView alloc]initWithFrame:CGRectMake(self.view.bounds.size.width-12, (menpiao.bounds.size.height-14)/2, 9, 14)];
    accessory1.image=[UIImage imageNamed:@"tongyong_arrow"];
    [menpiao addSubview:accessory1];
    
    weizhi=[UIButton buttonWithType:UIButtonTypeCustom];
    weizhi.frame=CGRectMake(0, menpiao.frame.origin.y+menpiao.frame.size.height, self.view.bounds.size.width, 40);
    [weizhi setBackgroundImage:[UIImage imageNamed:@"nearby_detail_line"] forState:UIControlStateNormal];
    [weizhi setTitle:[NSString stringWithFormat:@"   位置：%@",[infoDic objectForKey:@"m"]] forState:UIControlStateNormal];
...
-(void)weizhiButton
{
    WeiZhiVC *vc=[[WeiZhiVC alloc]initWithArray:@[[infoDic objectForKey:@"n"],[infoDic objectForKey:@"o"]] title:[infoDic objectForKey:@"m"]];
...
-(void)menpiaoButton
{
    MenPiaoDetailVC *vc=[[MenPiaoDetailVC alloc]init];
    vc.menpiaoDetail=[infoDic objectForKey:@"l"];
    [self.navigationController pushViewController:vc animated:YES];
}
